...
#subset the before vaccines
ids <- meta$Mouse_ID[which(meta$Vaccination_Timing == "After")]
pt6 <- pt6[,which(colnames(pt6) %in% ids)]
meta <- meta[which(meta$Mouse_ID %in% ids),]
...
#removing taxa with a total count of zero
#abund <- abund[-which(rowSums(abund) == 0),]

write.csv(abund, "results/After_Vaccination/all_formulas_after/taxonomy/genus/csvs/all_formuals_after_abundance.csv")

svg("results/all_formulas_after/taxonomy/genus/plots/pcoa/all_formuals_after.svg", onefile = TRUE, width = 10, height = 8)
# PCoA
...
bray_curtis_pcoa_df$Group <- factor(meta$Formula, levels = c("dmLT", "toxins_dmLT", "toxins_NTAs_dmLT"))

write.csv(bray_curtis_pcoa_df, "results/After_Vaccination/all_formulas_after/taxonomy/genus/csvs/all_formuals_after_pcoa.csv")
...
write.csv(pval.df, "results/all_formulas_after/taxonomy/genus/pvalues/beta_diversity/all_formulas_after.csv")
...
write.csv(kdf.ord, "results/all_formulas_after/taxonomy/genus/pvalues/diff_comparisons/all_formulas_after.csv")

#Dunn's test

pdf("results/all_formulas_after/taxonomy/genus/pvalues/diff_comparisons/all_formulas_before_after_dunn.pdf", onefile = TRUE, height = 10, width = 7)
new.kdf.ord <- kdf.ord[kdf.ord[,2] < 0.05,]
for(i in 1:dim(new.kdf.ord)[1]){
  print(c("this is the i",i))
  name <- rownames(new.kdf.ord)[i]
  dun1 <- dunn.test(as.numeric(abund[name,]), groups, method = "bh")
...
var1 <- "dmLT"
var2 <- "toxins_dmLT"
var3 <- "toxins_NTAs_dmLT"

vars <- c(var1, var2, var3)

#set up ids
for (i in 1:length(vars)){
  assign(paste0("ids.", i), meta$Mouse_ID[which(meta$Formula == vars[i])])
}
...
#all groups boxplots
pdf("results/all_formulas_after/taxonomy/genus/plots/box_plots/all_formulas_after.pdf", onefile = TRUE, width = 10, height = 7)
...
write.csv(combo, "results/After_Vaccination/all_formulas_after/taxonomy/genus/csvs/all_formuals_after_alpha_div.csv")

svg("results/all_formulas_after/taxonomy/genus/plots/box_plots/all_formulas_after_alpha_diversity.svg", onefile = TRUE, width = 10, height = 7)
...
#Dunn's test
dun1 <- dunn.test(as.numeric(alphadiv$alphadiv), groups, method = "bh")

whole <- rbind(dun1$comparisons, dun1$P.adjusted)
write.csv(whole, "results/all_formulas_after/taxonomy/genus/pvalues/diff_comparisons/all_formulas_after_alpha_diversity_dunn.csv")
