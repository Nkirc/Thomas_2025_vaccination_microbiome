...
#subset the dmLT formula samples
ids <- meta$Mouse_ID[which(meta$Formula == "toxins_NTAs_dmLT")]
pt6 <- pt6[,which(colnames(pt6) %in% ids)]
meta <- meta[which(meta$Mouse_ID %in% ids),]
...
classes <- meta$Vaccination_Timing
df <- abund
dist.meth <- "bray"   
...
write.csv(abund, "results/RI_Administration/before_vs_after_toxins_NTAs_dmLT_RI/taxonomy/genus/csvs/before_vs_after_toxins_NTAs_dmLT_RI_abundance.csv")

svg("results/before_vs_after_toxins_NTAs_dmLT_RI/taxonomy/genus/plots/pcoa/before_vs_after_toxins_NTAs_dmLT_RI.svg", onefile = TRUE, width = 10, height = 8)
# PCoA
...
bray_curtis_pcoa_df$Group <- factor(meta$Vaccination_Timing, levels = c("Before", "After"))

write.csv(bray_curtis_pcoa_df, "results/RI_Administration/before_vs_after_toxins_NTAs_dmLT_RI/taxonomy/genus/csvs/before_vs_after_toxins_NTAs_dmLT_RI_pcoa.csv")
...
write.csv(pval.df, "results/before_vs_after_toxins_NTAs_dmLT_RI/taxonomy/genus/pvalues/beta_diversity/before_vs_after_toxins_NTAs_dmLT_RI.csv")
...
write.csv(kdf.ord, "results/before_vs_after_toxins_NTAS_dmLT_RI/taxonomy/genus/pvalues/diff_comparisons/before_vs_after_toxins_NTAs_dmLT_RI.csv")
...
for (i in 1:length(vars)){
  assign(paste0("ids.", i), meta$Mouse_ID[which(meta$Vaccination_Timing == vars[i])])
}
...
#all groups boxplots
pdf("results/before_vs_after_toxins_NTAs_dmLT_RI/taxonomy/genus/plots/box_plots/before_vs_after_toxins_NTAs_dmLT_RI.pdf", onefile = TRUE, width = 10, height = 7)
...
alphadiv[,2] <- meta$Vaccination_Timing
...
write.csv(combo, "results/RI_Administration/before_vs_after_toxins_NTAs_dmLT_RI/taxonomy/genus/csvs/before_vs_after_toxins_NTAs_dmLT_RI_alpha_div.csv")

svg("results/before_vs_after_toxins_NTAs_dmLT_RI/taxonomy/genus/plots/box_plots/before_vs_after_toxins_NTAs_dmLT_RI_alpha_diversity.svg", onefile = TRUE, width = 10, height = 7)
