...
svg("results/pcoa_all_routes_all_formulas_before_vs_after.svg", onefile = TRUE, width = 10, height = 8)
# PCoA
...
bray_curtis_pcoa_df$Timing <- factor(meta$Vaccination_Timing, levels = c("Before", "After"))
bray_curtis_pcoa_df$Route <- factor(meta$Route, levels = c("IP", "RI"))

write.csv(bray_curtis_pcoa_df, "results/data_pcoa_all_routes_all_formulas_before_vs_after.csv")
...
# Creates a plot
plot <- ggplot(data = bray_curtis_pcoa_df, aes_string(x = "pcoa1", y = "pcoa2", color = "Group", fill = "Timing")) + 
  scale_fill_manual(values = c("white", "white")) +
  geom_point(size =5, aes(shape = interaction(Route, Timing))) +
  scale_shape_manual(values = c("IP.Before" = 21,
                                "IP.After" = 16,
                                "RI.Before" = 24,
                                "RI.After" = 17),
                     labels = c("IP", "RI"),
                     breaks = c("IP.After", "RI.After")) +
  ...
  guides(fill = guide_legend(override.aes = list(shape = c(21,16)))) +
  theme(
...
write.csv(pval.df, "results/RI_vs_IP_toxins_NTAs_dmLT_before/taxonomy/genus/pvalues/beta_diversity/RI_vs_IP_toxins_NTAs_dmLT_before.csv")
...
write.csv(kdf.ord, "results/RI_vs_IP_toxins_NTAs_dmLT_before/taxonomy/genus/pvalues/diff_comparisons/RI_vs_IP_toxins_NTAs_dmLT_before.csv")
...
#all groups boxplots
pdf("results/RI_vs_IP_toxins_NTAs_dmLT_before/taxonomy/genus/plots/box_plots/RI_vs_IP_toxins__NTAs_dmLT_before.pdf", onefile = TRUE, width = 10, height = 7)
...
pdf("results/RI_vs_IP_toxins_NTAs_dmLT_before/taxonomy/genus/plots/box_plots/RI_vs_IP_toxins_NTAs_dmLT_before_alpha_diversity.pdf", onefile = TRUE, width = 10, height = 7)
